...
const target=require('./launcher_verification_target').verificationTarget('profile-review-redesign');
(async()=>{
 ...
 try{
  ...
  for(const [width,height] of [[1440,900],[1024,680]]){
   ...
   await page.evaluate(() => openProfileDetail([...state.profiles.profiles].sort((a,b)=>b.applyChanges.length-a.applyChanges.length)[0]));
   await page.waitForSelector('.profile-change-group');
   assert(await page.$eval('.profile-detail-content', e => e.scrollWidth <= e.clientWidth + 1));
   assert(await page.$$eval('.profile-change-item', els => els.length) > 0);
   assert(await page.$eval('.profile-detail-dialog', e => e.getBoundingClientRect().bottom <= innerHeight));
   await page.screenshot({path:path.join(output,`review-${width}.png`)});
   const disclosure = await page.$('.profile-order-detail summary');
   if (disclosure) { await disclosure.click(); assert(await page.$eval('.profile-order-detail',e=>e.open)); }
   await page.evaluate(() => openProfileDetail({...state.profiles.profiles[0], label:'Normal Sweaty (Recommended)', applyChanges:[
    {key:'autoDodgeEnabled',label:'Auto Dodge',from:false,to:true},
    {key:'sessionBedwarsFields',label:'Session Bedwars Fields',from:['wins','games','finalDeaths','bedsLost','gamesByMode'],to:['wins','finalDeaths','bedsLost','games']},
    {key:'sessionDuelsFields',label:'Session Duels Fields',from:['wins','losses','gamesByMode'],to:['wins','losses','kills']},
